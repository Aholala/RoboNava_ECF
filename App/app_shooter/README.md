...
**数据流向：** `shooter_command + gimbal_feedback` --> `app_shooter` --> `module_shooter` --> `app_shooter_get_feedback()`
...
| `feedback` | `app_shooter_feedback_t` | 最近一次射击器反馈 |
...
| `app_shooter_update(me, command, gimbal, delta_time_s)` | 执行一个射击器控制周期：显式指令/云台反馈 -> 控制 -> 更新反馈 | `bsp_status_t` |
| `app_shooter_get_feedback(me)` | 读取最近射击器反馈 | 只读指针或 `NULL` |
...
当摩擦轮与拨弹机分布在不同板上时，具体项目读取远端状态，并在调用 `app_shooter_update()` 前调用 `module_shooter_set_external_friction_ready()`。`app_shooter` 不直接访问板间通信。
...
{
    app_shooter_update(&app_shooter, &command, &gimbal_feedback, dt_s);
}
```
...
4. **分体部署**：项目适配层负责板间通信，并通过 `module_shooter_set_external_friction_ready()` 同步远端状态。
